...
func NewLsTool(opts ...workspace.WorkspaceOption) (*toolcore.Tool, error) {
	...
	l := &LsTool{workspace: ws}
	return toolcore.NewTool(l.Info(), l.Run, toolcore.WithMiddleware(options.Middlewares...)), nil
}
...
func (l *LsTool) Run(ctx context.Context, call toolcore.ToolCall) (toolcore.ToolResponse, error) {
	var params LSParams
	...
	if _, err := os.Stat(searchPath); os.IsNotExist(err) {
		return toolcore.NewTextErrorResponse(fmt.Sprintf("path does not exist: %s", searchPath)), nil
	}

	files, truncated, err := listDirectory(searchPath, params.Ignore, MaxLSFiles)
	if err != nil {
		return toolcore.ToolResponse{}, fmt.Errorf("error listing directory: %w", err)
	}
...
func createFileTree(sortedPaths []string) []*TreeNode {
	...
	for _, path := range sortedPaths {
		hadTrailingSeparator := strings.HasSuffix(path, "/") || strings.HasSuffix(path, string(filepath.Separator))
		normalizedPath := filepath.Clean(filepath.FromSlash(path))
		...
		for i, part := range parts {
			...
			isDir := !isLastPart || hadTrailingSeparator
			nodeType := "file"
			if isDir {
				nodeType = "directory"
			}

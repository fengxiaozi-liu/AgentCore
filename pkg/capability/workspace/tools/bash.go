...
import (
	...
	"ferryman-agent/pkg/utils/shell"
)
...
func bashDescription() string {
	...
	return fmt.Sprintf(bashDescriptionTemplate, bannedCommandsStr, MaxOutputLength)
}

func NewBashTool(opts ...workspace.WorkspaceOption) (*toolcore.Tool, error) {
	...
	return toolcore.NewTool(b.Info(), b.Run, toolcore.WithMiddleware(options.Middlewares...)), nil
}

func (b *BashTool) Info() *toolcore.ToolInfo {
...
func (b *BashTool) Run(ctx context.Context, call toolcore.ToolCall) (toolcore.ToolResponse, error) {
	...
	if params.Command == "" {
		return toolcore.NewTextErrorResponse("missing command"), nil
	}

	baseCmd := strings.Fields(params.Command)[0]
	for _, banned := range bannedCommands {
		if strings.EqualFold(baseCmd, banned) {
			return toolcore.NewTextErrorResponse(fmt.Sprintf("command '%s' is not allowed", baseCmd)), nil
		}
	}
	...
	if stdout == "" {
		return toolcore.WithResponseMetadata(toolcore.NewTextResponse("no output"), metadata), nil
	}
	return toolcore.WithResponseMetadata(toolcore.NewTextResponse(stdout), metadata), nil
}

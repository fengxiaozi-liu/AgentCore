...
func NewSourcegraphTool(opts ...workspace.WorkspaceOption) *toolcore.Tool {
	t := &SourcegraphTool{
		client: &http.Client{
	...
	return toolcore.NewTool(t.Info(), t.Run, toolcore.WithMiddleware(options.Middlewares...))
}

func (t *SourcegraphTool) Info() *toolcore.ToolInfo {
...
func (t *SourcegraphTool) Run(ctx context.Context, call toolcore.ToolCall) (toolcore.ToolResponse, error) {
	var params SourcegraphParams
	if err := json.Unmarshal([]byte(call.Input), &params); err != nil {
		return toolcore.NewTextErrorResponse("Failed to parse sourcegraph parameters: " + err.Error()), nil
	}

	if params.Query == "" {
		return toolcore.NewTextErrorResponse("Query parameter is required"), nil
	}
	...
	if err != nil {
		return toolcore.ToolResponse{}, fmt.Errorf("failed to marshal GraphQL request: %w", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		if len(body) > 0 {
			return toolcore.NewTextErrorResponse(fmt.Sprintf("Request failed with status code: %d, response: %s", resp.StatusCode, string(body))), nil
		}
	...
	if err != nil {
		return toolcore.ToolResponse{}, fmt.Errorf("failed to read response body: %w", err)
	}

	var result map[string]any
	if err = json.Unmarshal(body, &result); err != nil {
		return toolcore.ToolResponse{}, fmt.Errorf("failed to unmarshal response: %w", err)
	}

	formattedResults, err := formatSourcegraphResults(result, params.ContextWindow)
	if err != nil {
		return toolcore.NewTextErrorResponse("Failed to format results: " + err.Error()), nil
	}

	return toolcore.NewTextResponse(formattedResults), nil
}
